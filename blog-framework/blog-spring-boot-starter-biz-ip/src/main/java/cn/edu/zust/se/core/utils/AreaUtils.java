package cn.edu.zust.se.core.utils;

import cn.edu.zust.se.core.Area;
import cn.edu.zust.se.core.enums.AreaEnum;
import cn.edu.zust.se.util.object.ObjectUtils;
...
import static cn.edu.zust.se.util.collection.CollectionUtils.convertList;
import static cn.edu.zust.se.util.collection.CollectionUtils.findFirst;
...
public class AreaUtils {
    ...
    private AreaUtils(){
        ...
        for (CsvRow row:rows){
            ...
            area.getChildren().add(area);
        }
        log.info("启动加载AreaUtils 成功，耗时 ({}) 毫秒", System.currentTimeMillis() - now);
    }
     ...
     * @return 区域列表
     */
    public static <T> List<T> getByType(AreaEnum type, Function<Area,T> func){
     ...
     * @return 上级区域编号
     */
    public static Integer getParentIdByType(Integer id, @NonNull AreaEnum type){
